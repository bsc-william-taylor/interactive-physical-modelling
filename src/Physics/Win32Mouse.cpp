...
void Win32Mouse::outputInput(Scene * scene)
{
	...
	for (auto i = 0; i < 3; i++)
	{
		...
		{
			scene->onMousePress(i, PRESSED, info.x, rect.bottom - info.y);
			mouseRelease[i] = true;
			releaseMsg[i] = false;
		}
		...
		if((state.rgbButtons[i] & 0x80) && mouseRelease[i])
		{
			scene->onMousePress(i, HOLDING, info.x, rect.bottom - info.y);
			releaseMsg[i] = false;
		}

		if(!(state.rgbButtons[i] & 0x80) && mouseRelease[i])
		{
			scene->onMousePress(i, RELEASED, info.x, rect.bottom - info.y);
